...
def automate_easemytrip():
    with sync_playwright() as p:
        ...
        page.locator("#txtSrcCity").click()
        page.locator("#txtSrcCity").press_sequentially("Delhi", delay=100)
        page.wait_for_function(
            "() => [...document.querySelectorAll('.auto-sugg-pre ul li')].some(el => el.innerText.trim().length > 0)",
            timeout=10000
        )
        ...
        page.locator("#txtDesCity").click()
        page.locator("#txtDesCity").press_sequentially("Shimla", delay=100)
        page.wait_for_function(
            "() => [...document.querySelectorAll('.auto-sugg-pre ul li')].some(el => el.innerText.trim().length > 0)",
            timeout=10000
        )
        ...
        time.sleep(1)
        print(f"[OK] Date selected: {target_date.strftime('%d %B %Y')}")
        ...
        time.sleep(5)
        print("[OK] Bus list loaded!")
        
        # ===== BUS TYPE FILTERS =====
        print("\n[SEARCH] === BUS TYPE FILTERS ===\n")
        time.sleep(2)

        def apply_and_deselect(label, selector):
            ...
            try:
                ...
                time.sleep(3)
                elem.click(force=True)
                print(f"   [OK] {label} deselected!")
                time.sleep(3)
            except Exception as e:
                print(f"   [WARN] {label} error: {e}")
            print("")

        apply_and_deselect("AC", "#disAc")
        apply_and_deselect("Non AC", "#disNonAc")
        apply_and_deselect("Sleeper", "#disSleeper")

        # Seater select karo aur scroll karo (deselect nahi)
        print("[>] Selecting Seater...")
        try:
            seater = page.locator("#disSeater").first
            seater.wait_for(state='visible', timeout=5000)
            seater.click(force=True)
            print("   [OK] Seater selected!")
            time.sleep(3)
        except Exception as e:
            print(f"   [WARN] Seater error: {e}")
        print("")
        
        print("")
        time.sleep(3)
        
        # ===== SCROLL THROUGH BUS LISTING =====
        print("\n[LIST] === SCROLLING THROUGH BUS LISTING ===\n")
        time.sleep(2)
        
        try:
            bus_listing = page.locator('.gen-cards').first
            bus_listing.hover()
        except:
            pass
