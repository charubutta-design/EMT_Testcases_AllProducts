...
def launch_easemytrip():
    with sync_playwright() as p:
        ...
        # Click on returndateTime
        print("Clicking on returndateTime...")
        return_datetime = page.locator("#returndateTime")
        return_datetime.wait_for(state="visible", timeout=10000)
        return_datetime.click()
        page.wait_for_load_state("domcontentloaded")
        print(f"Return DateTime Page Title: {page.title()}")
        page.screenshot(path="easemytrip_returndatetime.png")
        print("Screenshot saved as easemytrip_returndatetime.png")
