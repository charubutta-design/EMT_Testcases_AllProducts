...
def search_oneway_flight():
    with sync_playwright() as p:
        ...
        # Open EaseMyTrip
        page.goto("https://www.easemytrip.co.th/", timeout=60000)
        ...
        try:
            # First, try to close any overlays that might be blocking the button
            page.evaluate("""
                () => {
            ...
            if search_button.is_visible():
                ...
                page.evaluate("document.querySelector('.srchBtnSe').click()")
                print("Step 11: Clicked Search button using JavaScript (fallback)")
            time.sleep(5)
        except Exception as e:
            print(f"Step 11: Error clicking Search button: {e}")
            # Force click using JavaScript even if there's an error
            try:
                page.evaluate("document.querySelector('.srchBtnSe').click()")
                print("Step 11: Force-clicked Search button using JavaScript")
                time.sleep(5)
            except:
                print("Step 11: Failed to click Search button")
                time.sleep(5)
        ...
        print("Step 27: Clicking on PayPal payment method...")
        try:
            # Find PayPal payment option with class "paypal-txt PG2"
            paypal_button = page.locator(".paypal-txt.PG2")
            if paypal_button.count() > 0:
                # Scroll into view
                page.evaluate("""
                    (el) => {
                        el.scrollIntoView(true);
                    }
                """, paypal_button.first.element_handle())
                time.sleep(1)
                
                # Click on PayPal button
                paypal_button.first.click()
                print("Step 27: Clicked on PayPal payment method")
                time.sleep(3)
            else:
                # Try alternative selectors
                paypal_alt = page.locator("//div[contains(@class, 'paypal-txt')] | //button[contains(@class, 'paypal')] | //*[contains(text(), 'PayPal')]")
                if paypal_alt.count() > 0:
                    page.evaluate("""
                        (el) => {
                            el.scrollIntoView(true);
                        }
                    """, paypal_alt.first.element_handle())
                    time.sleep(1)
                    paypal_alt.first.click()
                    print("Step 27: Clicked on PayPal payment method (alternative selector)")
                    time.sleep(3)
                else:
                    # Try using JavaScript
                    page.evaluate("""
                        () => {
                            ...
                            if(paypalBtn) {
                                paypalBtn.scrollIntoView(true);
                                paypalBtn.click();
                                return true;
                            }
                            // Try alternative selectors
                            let elements = document.querySelectorAll('[class*="paypal"]');
                    ...
                    """)
                    print("Step 27: Clicked on PayPal payment method using JavaScript")
                    time.sleep(3)
        except Exception as e:
            print(f"Step 27: Error clicking PayPal payment method: {e}")
        
        # Take screenshot of payment page after PayPal selection
        print("Step 28: Taking screenshot of payment page after PayPal selection...")
        try:
            import os
            ...
            screenshot_path = os.path.join(screenshot_dir, f"payment_page_paypal_{timestamp}.png")
            
            page.screenshot(path=screenshot_path)
            print(f"Step 28: Screenshot saved to {screenshot_path}")
        except Exception as e:
            print(f"Step 28: Error taking screenshot: {e}")
        
        # Click on PayPal validation button with ng-click="MakevalidatepAYPAL()"
        print("Step 29: Clicking on PayPal validation button...")
        try:
            # Find element with ng-click="MakevalidatepAYPAL()"
            paypal_validate_btn = page.locator("[ng-click='MakevalidatepAYPAL()']")
            if paypal_validate_btn.count() > 0:
                # Scroll into view
                page.evaluate("""
                    (el) => {
                        el.scrollIntoView(true);
                    }
                """, paypal_validate_btn.first.element_handle())
                time.sleep(1)
                
                # Click on PayPal validation button
                paypal_validate_btn.first.click()
                print("Step 29: Clicked on PayPal validation button (ng-click='MakevalidatepAYPAL()')")
                time.sleep(3)
            else:
                # Try alternative selectors
                paypal_validate_alt = page.locator("//button[@ng-click='MakevalidatepAYPAL()'] | //div[@ng-click='MakevalidatepAYPAL()'] | //*[@ng-click*='MakevalidatepAYPAL']")
                if paypal_validate_alt.count() > 0:
                    page.evaluate("""
                        (el) => {
                            el.scrollIntoView(true);
                        }
                    """, paypal_validate_alt.first.element_handle())
                    time.sleep(1)
                    paypal_validate_alt.first.click()
                    print("Step 29: Clicked on PayPal validation button (alternative selector)")
                    time.sleep(3)
                else:
                    # Try using JavaScript
                    page.evaluate("""
                        () => {
                            let validateBtn = document.querySelector("[ng-click='MakevalidatepAYPAL()']");
                            if(validateBtn) {
                                validateBtn.scrollIntoView(true);
                                validateBtn.click();
                                console.log('Clicked PayPal validation button via JavaScript');
                                return true;
                            }
                            // Try to find by partial ng-click match
                            let elements = Array.from(document.querySelectorAll('[ng-click*="MakevalidatepAYPAL"]'));
                            if(elements.length > 0) {
                                elements[0].scrollIntoView(true);
                                elements[0].click();
                                console.log('Clicked PayPal validation button via alternative JavaScript');
                                return true;
                            }
                            return false;
                        }
                    """)
                    print("Step 29: Clicked on PayPal validation button using JavaScript")
                    time.sleep(3)
        except Exception as e:
            print(f"Step 29: Error clicking PayPal validation button: {e}")
        
        # Take screenshot after PayPal validation
        print("Step 30: Taking screenshot after PayPal validation...")
        try:
            import os
            screenshot_dir = r"D:\EMT_Flight_Playwright\Test_cases\ScreenShot"
            ...
            # Take screenshot with timestamp
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            screenshot_path = os.path.join(screenshot_dir, f"payment_page_after_validation_{timestamp}.png")
            
            page.screenshot(path=screenshot_path)
            print(f"Step 30: Screenshot saved to {screenshot_path}")
        except Exception as e:
            print(f"Step 30: Error taking screenshot: {e}")
        
        # Final screenshot after all actions
        print("Step 31: Taking final screenshot...")
        try:
            import os
            screenshot_dir = r"D:\EMT_Flight_Playwright\Test_cases\ScreenShot"
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            final_screenshot_path = os.path.join(screenshot_dir, f"final_payment_page_{timestamp}.png")
            
            page.screenshot(path=final_screenshot_path)
            print(f"Step 31: Final screenshot saved to {final_screenshot_path}")
            
            # List all screenshots in the directory
            screenshots = [f for f in os.listdir(screenshot_dir) if f.endswith('.png')]
            print(f"\nStep 31: Total screenshots saved: {len(screenshots)}")
            print(f"Step 31: Screenshots directory: {screenshot_dir}")
            for screenshot in sorted(screenshots):
                print(f"  - {screenshot}")
        except Exception as e:
            print(f"Step 31: Error taking final screenshot: {e}")
