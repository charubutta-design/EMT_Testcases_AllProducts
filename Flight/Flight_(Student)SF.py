...
def navigate_to_flights():
    with sync_playwright() as p:
        ...
        # Click on 'Students' (class="container_df")
        try:
            print("Clicking on Students...")
            students = page.locator('label.container_df:has-text("Students")')
            if students.is_visible(timeout=2000):
                students.click()
                print("Successfully clicked on Students")
                page.wait_for_timeout(1000)
            else:
                print("Students element not found or not visible")
        except Exception as e:
            print(f"Error clicking on Students: {e}")
        ...
        # Wait for listing page to load and click on  SpiceJet checkbox
        try:
            print("Waiting for listing page to load...")
            # Wait for the filter section to appear
            page.wait_for_selector('label:has-text("SpiceJet")', timeout=20000)
            print("✓ Listing page loaded - found SpiceJet option")
            page.wait_for_timeout(2000)
            
            # Look for SpiceJet within the Airlines filter
            spicejet = page.locator('label:has-text("SpiceJet")').first
            
            if spicejet.is_visible(timeout=2000):
                print("SpiceJet option is visible")
                
                # Find the associated checkbox within the label
                checkbox = spicejet.locator('.checkmark_lgn')
                
                if checkbox.count() > 0:
                    checkbox.click()
                    print("✓ Successfully clicked on SpiceJet checkbox")
                    page.wait_for_timeout(2000)
                else:
                    # Try clicking the label directly
                    spicejet.click()
                    print("✓ Clicked on SpiceJet label")
                    page.wait_for_timeout(2000)
            else:
                print("SpiceJet option not visible")
                
        except Exception as e:
            print(f"Error selecting SpiceJet: {e}")
        ...
        # Find and select 'StudentFare' fare and click the Book Now button
        try:
            print("Looking for StudentFare fare option...")
            page.wait_for_timeout(2000)
            
            # Find the StudentFare option
            student_fare_label = page.locator('label:has-text("StudentFare")').first
            
            if student_fare_label.is_visible(timeout=3000):
                print("✓ Found StudentFare label")
                
                # Scroll into view
                student_fare_label.scroll_into_view_if_needed()
                page.wait_for_timeout(500)
                
                # Find the parent container - try multiple class names
                parent_container = student_fare_label.locator('xpath=./ancestor::div[contains(@class, "flightOpt") or contains(@class, "flight-option") or contains(@class, "fare-option") or contains(@class, "option-row") or contains(@class, "flight-result")]').first
                
                if parent_container.count() == 0:
                    # If not found, get the closest parent with significant width/padding
                    parent_container = student_fare_label.locator('xpath=./ancestor::div[self::div[contains(@style, "display")]]').first
                
                if parent_container.count() == 0:
                    # Last resort - get the grandparent
                    parent_container = student_fare_label.locator('xpath=./ancestor::div[2]').first
                
                print(f"Parent container found: {parent_container.count() > 0}")
                page.screenshot(path="D:\\EMT_Flight_Playwright\\Test_cases\\ScreenShot\\student_fare_parent.png")
                
                if parent_container.count() > 0:
                    # Click the radio button
                    radio = student_fare_label.locator('input[type="radio"]').first
                    if radio.count() == 0:
                        radio = student_fare_label.locator('xpath=./preceding-sibling::input[@type="radio"]').first
                    
                    if radio.count() > 0:
                        try:
                            radio.click()
                            print("✓ Clicked StudentFare radio button")
                            page.wait_for_timeout(1000)
                        except Exception as e:
                    ...
                    for i in range(min(10, all_buttons.count())):  # Check first 10 buttons
                        ...
                        try:
                            ...
                            if 'book' in btn_text.lower() and 'now' in btn_text.lower():
                                ...
                                page.screenshot(path="D:\\EMT_Flight_Playwright\\Test_cases\\ScreenShot\\before_student_fare_book_now.png")
                                print("Screenshot saved: before_student_fare_book_now.png")
                                btn.click()
                                print("✓ Successfully clicked Book Now for StudentFare fare")
                                page.wait_for_timeout(3000)
                    ...
                    if book_button_found:
                        ...
                        # Enter Student Id with conditional logic
                        try:
                            print("Looking for student ID field...")
                            
                            # Take a screenshot before searching
                            page.screenshot(path="D:\\EMT_Flight_Playwright\\Test_cases\\ScreenShot\\before_student_id_search.png")
                            print("Screenshot saved: before_student_id_search.png")
                            
                            # Try different selectors to find the element
                            # Method 1: Find by ID (without tag restriction)
                            student_id_field = page.locator('#txtDocumentIdAdult0').first
                            
                            # Method 2: If not found, try with div tag
                            if student_id_field.count() == 0:
                                student_id_field = page.locator('div#txtDocumentIdAdult0').first
                            
                            # Method 3: If still not found, look for any element with that ID
                            if student_id_field.count() == 0:
                                student_id_field = page.locator('[id="txtDocumentIdAdult0"]').first
                            
                            # Method 4: Look for input inside the div
                            if student_id_field.count() == 0:
                                student_id_field = page.locator('#txtDocumentIdAdult0 input').first
                            
                            print(f"Student ID field found: {student_id_field.count() > 0}")
                            
                            if student_id_field.count() > 0:
                                print("✓ Found student ID field (id='txtDocumentIdAdult0')")
                                
                                # Scroll into view
                                student_id_field.scroll_into_view_if_needed()
                                page.wait_for_timeout(300)
                                ...
                                student_id_field.fill("123ABC")
                                print("✓ Successfully entered '123ABC' in student ID field")
                                page.wait_for_timeout(500)
                            else:
                                print("✗ FAILED: Student ID field (id='txtDocumentIdAdult0') not found")
                                page.screenshot(path="D:\\EMT_Flight_Playwright\\Test_cases\\ScreenShot\\student_id_not_found.png")
                                print("Debug screenshot saved: student_id_not_found.png")
                                # Fail the test case
                                raise Exception("Test Case FAILED: Required element 'txtDocumentIdAdult0' not found on the page")
                            ...
                            if "Test Case FAILED" in str(e):
                                ...
                                print(f"Error entering student ID: {e}")
                                import traceback
                                traceback.print_exc()
                                sys.exit(1)
