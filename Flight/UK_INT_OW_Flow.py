...
def search_oneway_flight():
    with sync_playwright() as p:
        ...
        # Open EaseMyTrip
        page.goto("https://www.easemytrip.co.uk/", timeout=60000)
        ...
        try:
            # Remove overlay barriers that might block clicking
            page.evaluate("""
                () => {
                    ...
                    let overlay2 = document.querySelector('.overlaybg1');
                    if(overlay2) {
        ...
        page.wait_for_load_state("domcontentloaded")
        time.sleep(5)  # Extra wait for page elements to render
        try:
            page.wait_for_load_state("networkidle", timeout=10000)
        except:
            print("Step 14: networkidle timeout, continuing anyway")
            time.sleep(3)
        time.sleep(3)
        ...
        try:
            ...
            if email_field.is_visible():
                ...
                # Type the email
                email_field.type("manish.dasila@easemytrip.co.uk", delay=50)
                print("Step 19: Entered 'manish.dasila@easemytrip.co.uk' in Email Address field")
        ...
        # Click on PayPal payment option
        print("Step 27: Clicking on PayPal payment option...")
        
        try:
            # Remove overlay barriers that might block clicking
            page.evaluate("""
                () => {
                    let overlays = document.querySelectorAll('[class*="overlay"], [id*="overlay"], ._onerohdr, .emt_listrgt');
                    overlays.forEach(el => {
            ...
            # Try to click PayPal using the correct selectors
            paypal_clicked = False
            
            # Method 1: Click the pay_pal div directly
            try:
                paypal_div = page.locator(".pay_pal").first
                if paypal_div.is_visible():
                    paypal_div.click(timeout=10000)
                    print("Step 27: Clicked on PayPal payment option using .pay_pal selector")
                    paypal_clicked = True
                    time.sleep(3)
            except:
                pass
            
            # Method 2: Click using pay_paltxt span
            if not paypal_clicked:
                try:
                    paypal_span = page.locator(".pay_paltxt").first
                    if paypal_span.is_visible():
                        paypal_span.click(timeout=10000)
                        print("Step 27: Clicked on PayPal payment option using .pay_paltxt selector")
                        paypal_clicked = True
                        time.sleep(3)
                except:
                    pass
            
            # Method 3: JavaScript click on pay_pal div
            if not paypal_clicked:
                page.evaluate("""
                    () => {
                        let paypalDiv = document.querySelector('.pay_pal');
                        if(paypalDiv) {
                            console.log('Clicking PayPal div');
                            paypalDiv.click();
                            return true;
                        }
                    }
                """)
                print("Step 27: Clicked on PayPal payment option using JavaScript (.pay_pal)")
                paypal_clicked = True
                time.sleep(3)
                
        except Exception as e:
            print(f"Step 27: Error clicking PayPal payment option: {e}")
        
        # Wait for PayPal page to load
        print("Step 28: Waiting for PayPal page to load...")
        try:
            page.wait_for_load_state("domcontentloaded")
        except:
            pass
        time.sleep(3)
        print("Step 28: PayPal page loaded successfully!")
        
        # Wait for PayPal redirect or form to load
        print("Step 29: Waiting for PayPal checkout to load...")
        try:
            page.wait_for_load_state("domcontentloaded")
        except:
            pass
        time.sleep(3)
        print("Step 29: PayPal checkout loaded successfully!")
        
        # Wait for next page to load
        print("Step 30: Waiting for PayPal checkout processing...")
        try:
            page.wait_for_load_state("domcontentloaded")
        except:
            pass
        time.sleep(3)
        print("Step 30: PayPal checkout processing initiated!")
        
        # Click on PayPal validator button - ng-click="MakevalidatepAYPAL()"
        print("Step 31: Clicking on PayPal validator button...")
        
        try:
            # Find and click the element with ng-click="MakevalidatepAYPAL()"
            paypal_validator = page.locator('[ng-click="MakevalidatepAYPAL()"]').first
            if paypal_validator.is_visible():
                paypal_validator.click(timeout=10000)
                print("Step 31: Clicked on PayPal validator using ng-click selector")
                time.sleep(3)
            else:
                print("Step 31: PayPal validator button not visible, trying JavaScript...")
                # Use JavaScript to find and click the element
                page.evaluate("""
                    () => {
                        let btn = document.querySelector('[ng-click="MakevalidatepAYPAL()"]');
                        if(btn) {
                            console.log('Found PayPal validator button');
                            btn.scrollIntoView(true);
                            btn.click();
                            return true;
                        }
                        // Alternative: search by partial text match
                        let buttons = Array.from(document.querySelectorAll('button, div, span'));
                        for(let b of buttons) {
                            if(b.getAttribute && b.getAttribute('ng-click') && 
                               b.getAttribute('ng-click').includes('Validate') && 
                               b.getAttribute('ng-click').includes('PAYPAL')) {
                                console.log('Found via ng-click attribute search');
                ...
                """)
                print("Step 31: Clicked on PayPal validator using JavaScript")
                time.sleep(3)
        except Exception as e:
            print(f"Step 31: Error clicking PayPal validator: {e}")
        
        # Click on Make Payment button
        print("Step 32: Clicking on Make Payment button...")
        ...
        try:
            ...
            if make_payment_id.is_visible():
                ...
                print("Step 32: Clicked on Make Payment button")
                time.sleep(3)
            else:
                print("Step 32: Make Payment button not visible with ID, trying alternative selectors...")
                ...
                if make_payment_btn.is_visible():
                    ...
                    print("Step 32: Clicked on Make Payment button (div selector)")
                    time.sleep(3)
                else:
                    # Try with button tag
                    ...
                    if make_payment_alt.is_visible():
                        ...
                        print("Step 32: Clicked on Make Payment button (button selector)")
                        time.sleep(3)
                    else:
                        # Try using JavaScript
                        page.evaluate("""
                        ...
                        """)
                        print("Step 32: Clicked on Make Payment button using JavaScript")
                        time.sleep(3)
        except Exception as e:
            print(f"Step 32: Error clicking Make Payment button: {e}")
        
        # Click on Cancel and return to EaseMyTrip UK Ltd button before taking screenshot
        print("Step 33: Clicking on 'Cancel and return to EaseMyTrip UK Ltd' button...")
        try:
            cancel_link = page.locator("#cancelLink")
            if cancel_link.is_visible():
                cancel_link.click()
                print("Step 33: Clicked on cancelLink button")
                time.sleep(2)
            else:
                print("Step 33: cancelLink button not visible")
        except Exception as e:
            print(f"Step 33: Error clicking cancelLink: {e}")
        
        # Take screenshot after clicking cancel
        print("Step 34: Taking screenshot...")
        try:
            import os
            ...
            print(f"Step 34: Screenshot saved to {screenshot_path}")
        except Exception as e:
            print(f"Step 34: Error taking screenshot: {e}")
