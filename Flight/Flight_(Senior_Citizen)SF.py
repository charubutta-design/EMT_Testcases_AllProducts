...
def navigate_to_flights():
    with sync_playwright() as p:
        ...
        # Click on 'Senior Citizens' (class="container_df")
        try:
            print("Clicking on Senior Citizens...")
            senior_citizens = page.locator('label.container_df:has-text("Senior Citizens")')
            if senior_citizens.is_visible(timeout=2000):
                senior_citizens.click()
                print("Successfully clicked on Senior Citizens")
                page.wait_for_timeout(1000)
            else:
                print("Senior Citizens element not found or not visible")
        except Exception as e:
            print(f"Error clicking on Senior Citizens: {e}")
        ...
        # Wait for listing page to load and click on IndiGo checkbox
        try:
            print("Waiting for listing page to load...")
            # Wait for the filter section to appear
            page.wait_for_selector('label:has-text("IndiGo")', timeout=20000)
            print("✓ Listing page loaded - found IndiGo option")
            page.wait_for_timeout(2000)
            
            # Look for IndiGo within the Airlines filter
            indigo = page.locator('label:has-text("IndiGo")').first
            
            if indigo.is_visible(timeout=2000):
                print("IndiGo option is visible")
                
                # Find the associated checkbox within the label
                checkbox = indigo.locator('.checkmark_lgn')
                
                if checkbox.count() > 0:
                    checkbox.click()
                    print("✓ Successfully clicked on IndiGo checkbox")
                    page.wait_for_timeout(2000)
                else:
                    # Try clicking the label directly
                    indigo.click()
                    print("✓ Clicked on IndiGo label")
                    page.wait_for_timeout(2000)
            else:
                print("IndiGo option not visible")
                
        except Exception as e:
            print(f"Error selecting IndiGo: {e}")
        ...
        # Find and select 'Senior Citizens' fare and click the Book Now button
        try:
            print("Looking for SeniorCitizen fare option...")
            page.wait_for_timeout(2000)
            
            # Find the SeniorCitizen option
            senior_citizen_label = page.locator('label:has-text("SeniorCitizen")').first
            
            if senior_citizen_label.is_visible(timeout=3000):
                print("✓ Found SeniorCitizen label")
                
                # Scroll into view
                senior_citizen_label.scroll_into_view_if_needed()
                page.wait_for_timeout(500)
                
                # Find the parent container - try multiple class names
                parent_container = senior_citizen_label.locator('xpath=./ancestor::div[contains(@class, "flightOpt") or contains(@class, "flight-option") or contains(@class, "fare-option") or contains(@class, "option-row") or contains(@class, "flight-result")]').first
                
                if parent_container.count() == 0:
                    # If not found, get the closest parent with significant width/padding
                    parent_container = senior_citizen_label.locator('xpath=./ancestor::div[self::div[contains(@style, "display")]]').first
                
                if parent_container.count() == 0:
                    # Last resort - get the grandparent
                    parent_container = senior_citizen_label.locator('xpath=./ancestor::div[2]').first
                
                print(f"Parent container found: {parent_container.count() > 0}")
                page.screenshot(path="D:\\EMT_Flight_Playwright\\Test_cases\\ScreenShot\\senior_citizen_parent.png")
                
                if parent_container.count() > 0:
                    # Click the radio button
                    radio = senior_citizen_label.locator('input[type="radio"]').first
                    if radio.count() == 0:
                        radio = senior_citizen_label.locator('xpath=./preceding-sibling::input[@type="radio"]').first
                    
                    if radio.count() > 0:
                        try:
                            radio.click()
                            print("✓ Clicked SeniorCitizen radio button")
                            page.wait_for_timeout(1000)
                        except Exception as e:
                    ...
                    for i in range(min(10, all_buttons.count())):  # Check first 10 buttons
                        ...
                        try:
                            ...
                            if 'book' in btn_text.lower() and 'now' in btn_text.lower():
                                ...
                                page.screenshot(path="D:\\EMT_Flight_Playwright\\Test_cases\\ScreenShot\\before_senior_citizen_book_now.png")
                                print("Screenshot saved: before_senior_citizen_book_now.png")
                                btn.click()
                                print("✓ Successfully clicked Book Now for SeniorCitizen fare")
                                page.wait_for_timeout(3000)
                    ...
                    if book_button_found:
                        ...
                        # Verify Date of Birth (DOB) field with conditional logic
                        try:
                            print("Looking for Date of Birth field...")
                            
                            # Verify the presence of the 'Date of Birth' field (class = "lbl")
                            dob_label = page.locator('[class="lbl"]:has-text("Date of Birth")').first
                            
                            print(f"Date of Birth field found: {dob_label.count() > 0}")
                            
                            if dob_label.count() == 0:
                                print("✗ FAILED: Date of Birth (class='lbl') field not found")
                                page.screenshot(path="D:\\EMT_Flight_Playwright\\Test_cases\\ScreenShot\\dob_field_not_found.png")
                                print("Debug screenshot saved: dob_field_not_found.png")
                                # Fail the test case
                                raise Exception("Test Case FAILED: Required element 'Date of Birth' field not found on the page")
                            else:
                                print("✓ Found Date of Birth field (class='lbl')")
                                
                                # Click on id="divDOBDayAdult0" and select 10
                                dob_day_field = page.locator('#divDOBDayAdult0').first
                                if dob_day_field.count() > 0:
                                    print("✓ Found DOB Day field (id='divDOBDayAdult0')")
                                    dob_day_field.scroll_into_view_if_needed()
                                    page.wait_for_timeout(300)
                                    dob_day_field.click()
                                    page.wait_for_timeout(500)
                                    
                                    # Select 10 using keyboard input
                                    page.keyboard.type("10")
                                    page.keyboard.press("Enter")
                                    print("✓ Successfully selected day '10'")
                                    page.wait_for_timeout(300)
                                else:
                                    raise Exception("DOB Day field (id='divDOBDayAdult0') not found")
                                
                                # Click on id="divDOBMonAdult0" and select May
                                dob_month_field = page.locator('#divDOBMonAdult0').first
                                if dob_month_field.count() > 0:
                                    print("✓ Found DOB Month field (id='divDOBMonAdult0')")
                                    dob_month_field.scroll_into_view_if_needed()
                                    page.wait_for_timeout(300)
                                    dob_month_field.click()
                                    page.wait_for_timeout(500)
                                    
                                    # Select May using keyboard input
                                    page.keyboard.type("May")
                                    page.keyboard.press("Enter")
                                    print("✓ Successfully selected month 'May'")
                                    page.wait_for_timeout(300)
                                else:
                                    raise Exception("DOB Month field (id='divDOBMonAdult0') not found")
                                
                                # Click on id="divDOBYarAdult0" and select 1964
                                dob_year_field = page.locator('#divDOBYarAdult0').first
                                if dob_year_field.count() > 0:
                                    print("✓ Found DOB Year field (id='divDOBYarAdult0')")
                                    dob_year_field.scroll_into_view_if_needed()
                                    page.wait_for_timeout(300)
                                    dob_year_field.click()
                                    page.wait_for_timeout(500)
                                    
                                    # Select 1964 from the dropdown using keyboard
                                    page.keyboard.type("1964")
                                    page.keyboard.press("Enter")
                                    print("✓ Successfully selected year '1964'")
                                    page.wait_for_timeout(300)
                                else:
                                    raise Exception("DOB Year field (id='divDOBYarAdult0') not found")
                                
                                print("✓ Successfully entered Date of Birth: 10 May 1964")
                                page.wait_for_timeout(500)
