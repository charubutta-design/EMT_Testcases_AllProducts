...
def search_flight():
    with sync_playwright() as p:
        ...
        page.goto("https://www.easemytrip.ae/")
        ...
        # Step 2: Click To field and select DXB
        try:
            page.type("input[placeholder='Destination City']", "DXB", delay=50)
            print("✓ To field: DXB typed")
            page.wait_for_timeout(600)
            page.locator("li").filter(has_text="Dubai").first.click(timeout=2000)
            print("✓ To: DXB selected")
        except Exception as e:
        ...
        # Click Swap button to exchange From and To cities
        print("🔍 STEP 1.5: Clicking Swap button (id='swap' class='swipesector')...\n")
        try:
            page.locator("#swap").click(timeout=5000, force=True)
            print("✓ Swap button clicked via Playwright")
            page.wait_for_timeout(800)
        except Exception as e:
            print(f"⚠ Swap click error: {e}")
            try:
                page.evaluate("""
                () => {
                    let swapBtn = document.getElementById('swap');
                    if (swapBtn) {
                        swapBtn.scrollIntoView({ behavior: 'smooth', block: 'center' });
                        swapBtn.click();
                    }
                }
                """)
                print("✓ Swap button clicked via JavaScript")
                page.wait_for_timeout(800)
            except Exception as e2:
                print(f"⚠ Swap button JavaScript click failed: {e2}")
        
        print(f"📍 URL after Swap: {page.url}\n")
        ...
        # Step 2.5: Click LoadMore button to load more flights
        print("🔍 STEP 2.5: Clicking LoadMore button (id='LoadMore')...\n")
        
        page.wait_for_timeout(1000)
        
        try:
            # Find LoadMore button info
            loadmore_info = page.evaluate("""
            () => {
            ...
            """)
            print(f"📌 LoadMore Button Info: {loadmore_info}\n")
            
            if loadmore_info.get('found'):
                # Try Playwright click first
                try:
                    page.locator("#LoadMore").click(timeout=5000, force=True)
                    print("✓ LoadMore button clicked via Playwright")
                    ...
                    try:
                        ...
                        () => {
                            ...
                            if (btn) {
                                ...
                                console.log('LoadMore button clicked via JavaScript');
                            }
                        }
                        """)
                        print("✓ LoadMore button clicked via JavaScript")
                        page.wait_for_timeout(2000)
                    except Exception as e2:
                        print(f"⚠ JavaScript click also failed: {e2}")
            else:
                print("⚠ LoadMore button not found on this page")
        except Exception as e:
            print(f"✗ Error clicking LoadMore: {e}")
        
        print(f"📍 URL after LoadMore: {page.url}\n")
        
        # Step 3: Wait for Book Now buttons to load and click the first one
        print("🔍 STEP 3: Clicking first Book Now button from flight listings...\n")
        
        page.wait_for_timeout(3000)
        
        try:
            # Wait for Book Now buttons to be rendered in the DOM
            max_wait = 10000  # 10 seconds
            start_time = page.evaluate("() => Date.now()")
            buttons_found = False
            
            while page.evaluate("() => Date.now()") - start_time < max_wait:
                book_buttons = page.evaluate("""
                () => {
                    let buttons = [];
                    let allButtons = document.querySelectorAll('button');
                    ...
                    allButtons.forEach((btn, index) => {
                        ...
                        if (text.includes('Book') && text.includes('Now') && btn.offsetParent !== null && !btn.disabled) {
                            buttons.push({
                                index: index,
                                className: btn.className,
                                id: btn.id || '',
            ...
            if buttons_found and book_buttons:
                print(f"📌 Found {len(book_buttons)} visible 'Book Now' button(s)")
                print(f"   First button: {book_buttons[0]}\n")
                
                # Click the first visible Book Now button
                page.evaluate("""
                () => {
                ...
                print("⚠ No visible 'Book Now' buttons found after waiting")
        except Exception as e:
            print(f"✗ Error finding/clicking Book Now buttons: {e}")
        ...
        # Step: Click on Continue button (class="book-bt-nwap marg-left") after More Fare page
        print("🔍 STEP 4.6: Clicking Continue button (book-bt-nwap marg-left)...\n")
        
        try:
            # Get Continue button info
            continue_button_info = page.evaluate("""
            () => {
                let button = document.querySelector('.book-bt-nwap.marg-left');
                if (!button) {
                    button = document.querySelector('[class*="book-bt-nwap"]');
                }
                if (!button) {
                    let buttons = document.querySelectorAll('button, div[role="button"], [class*="btn"]');
                    for (let btn of buttons) {
                        if (btn.className && btn.className.includes('book-bt-nwap')) {
                            return {
                                found: true,
                                tagName: btn.tagName,
                                className: btn.className,
            ...
            print("📌 Continue Button Info: " + str(continue_button_info) + "\n")
            
            if continue_button_info.get('found'):
                # Method 1: Try clicking via Playwright
                try:
                    page.locator(".book-bt-nwap.marg-left").first.click(timeout=5000)
                ...
                final_url = page.url
                print(f"✓ Current URL after Continue click: {final_url}\n")
                
            else:
                print("⚠ Continue button (book-bt-nwap marg-left) not found on page\n")
        ...
        try:
            ...
            if email_field_info.get('found'):
                ...
                try:
                    page.locator("#txtEmailId").fill("manish.dasila@easemytrip.ae")
                    print("✓ Email Address 'manish.dasila@easemytrip.ae' entered via fill()")
                except Exception as e:
                    print(f"⚠ fill() failed: {e}")
                    ...
                    try:
                        ...
                        page.locator("#txtEmailId").type("manish.dasila@easemytrip.ae", delay=25)
                        print("✓ Email Address 'manish.dasila@easemytrip.ae' entered via type()")
                    except Exception as e2:
                        print(f"⚠ type() also failed: {e2}")
        ...
        # Step 26: Take Screenshot of Payment Page
        print("🔍 STEP 26: Taking Screenshot of Payment Page...\n")
