...
def search_flight():
    with sync_playwright() as p:
        ...
        page.goto("https://www.easemytrip.co.uk/")
        page.wait_for_load_state("domcontentloaded")
        page.wait_for_timeout(1000)
        
        print("\n=== FLIGHT SEARCH AUTOMATION ===\n")
        
        # Step 1: Click From field and select London
        try:
            page.locator("#FromSector_show").click(force=True)
            print("✓ From field clicked")
            page.wait_for_timeout(500)
            page.type("#pff", " London", delay=50)
            page.wait_for_timeout(600)
            page.locator("li").filter(has_text="London").first.click()
            print("✓ From: London selected")
        ...
        try:
            page.type("input[placeholder='Destination City']", "MAN", delay=50)
            print("✓ To field: MAN typed")
            page.wait_for_timeout(600)
            page.locator("li").filter(has_text="Manchester").first.click(timeout=2000)
            print("✓ To: Manchester selected")
        except Exception as e:
